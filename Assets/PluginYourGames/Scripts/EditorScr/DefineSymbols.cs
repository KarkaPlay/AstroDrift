    ...
    public static class DefineSymbols
    {
        ...
        public static void PlatformDefineSymbols()
        {
            ...
            foreach (BuildTargetGroup buildTargetGroup in GetSupportedBuildTargetGroups())
            {
                ...
                string originalDefinesText = PlayerSettings.GetScriptingDefineSymbols(namedBuildTarget);

                string definesText = originalDefinesText;
                ...
                // Нормализация: trim + удаление пустых элементов, join ';'.
                // Сравнение с исходной строкой: SetScriptingDefineSymbols вызывается ТОЛЬКО при реальном изменении.
                // Это устраняет «Player settings modified» → Requested script compilation → domain reload в стабильном состоянии
                // и гасит возможную петлю projectChanged → Set → projectChanged (второй write становится no-op).
                definesText = NormalizeDefinesText(string.Join(";", defines));

                if (definesText != originalDefinesText)
                {
                    PlayerSettings.SetScriptingDefineSymbols(namedBuildTarget, definesText);
                }
            }
        ...
        public static void RemoveDefine(string define)
        {
            ...
            foreach (BuildTargetGroup buildTargetGroup in GetSupportedBuildTargetGroups())
            {
                ...
                // Contains — substring-проверка: «MY_YG» совпадает и с «MY_YG2».
                // Пишем только когда define присутствует как отдельный элемент списка.
                List<string> defineList = defines.Split(';').ToList();

                if (defineList.Contains(define))
                {
                    defineList.Remove(define);

                    string newDefines = NormalizeDefinesText(string.Join(";", defineList));
        ...
        /// <summary>
        /// Нормализация строки defines: split ';', trim элементов, удаление пустых, join ';'.
        /// Позволяет корректно сравнить «до/после» и избежать холостых записей SetScriptingDefineSymbols,
        /// каждая из которых помечает PlayerSettings dirty и вызывает домен-релоад.
        /// </summary>
        public static string NormalizeDefinesText(string definesText)
        {
            if (string.IsNullOrEmpty(definesText))
                return string.Empty;

            IEnumerable<string> cleaned = definesText
                .Split(';')
                .Select(d => d.Trim())
