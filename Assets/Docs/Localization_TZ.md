...
- Массив `Entries` в `AstroDriftLocalizationSetup.cs` **неполон**: ~20 ключей (`best`, `score`, `new_best`, `continue_*`, `pause_*`, `combo`, `shield_*`, `pickup_*`) созданы вне его. Валидатор не опирается на `Entries`. **Фактические правки (§1.6 — две итерации; вторая — по независимой перепроверке Team Lead на финальной приёмке §14):** правка №1 (задача 8) ошибочно заявила, что `title_main`, `title_sub`, `pilot_level` **все три** есть в `Entries`. Правка №2 (корректная): в `Entries` (63 ключа, [`AstroDriftLocalizationSetup.cs:22`](Assets/Scripts/Core/AstroDriftLocalizationSetup.cs:22)) находится **только `pilot_level`** ([`AstroDriftLocalizationSetup.cs:36`](Assets/Scripts/Core/AstroDriftLocalizationSetup.cs:36)); `title_main`/`title_sub` в `Entries` **никогда не были** — проверено по git-истории (`d9bf5e1`, `d90f06e`, `f0155fd`, `0a920d0`, HEAD: ноль вхождений). Эти два ключа существуют только в Shared Data — [`GameTexts Shared Data.asset:19`](Assets/Localizations/GameTexts Shared Data.asset:19) (id `1727750201344`) и `:23` (id `1728396124160`) — и имеют строки в обеих таблицах, но **не читаются ни одной нодой/полем** (§8.1/§8.2 их не назначают). `pilot_level` не читается так же — вытеснен `pilot_level_label`. Логотип меню — картинка ([`MenuLogo.prefab`](Assets/Prefabs/Menu/MenuLogo.prefab)). Источник №1 валидатора исключает `Entries` по построению, поэтому `pilot_level` в отчёт не попадает, а `title_main`/`title_sub` попадают как непрочитанные ключи таблиц. **Критерий «0 сирот» фактически не достигнут:** `orphans=3` = 1 ключ из `Entries` + 2 ключа только из Shared Data (P3, решение за продуктом).
...
6. **Coverage чарсета (от рецидива B1):** все кодпоинты всех строк таблиц обязаны существовать в назначенных Static-шрифтах (проверка через таблицу символов Font Asset'а). Новые символы без обновления чарсета — варн валидатора. **Трактовка «назначенных» (решение продюсера, задача 7): объединение назначенных шрифтов** — кодпоинт покрыт, если он есть **хотя бы в одном** шрифте, назначенном ролям (заполненные слоты `TypographyConfig`, плюс фиксированный набор fallback'ов из §7.1 п.6); отсутствие во всех — варн. Это осознанно слабее «во всех», иначе плейсхолдер-конфиг (все слоты → один guid) давал бы гарантированно красный отчёт.
...
- [x] Отчёт зелёный на RU/EN — **с квалификацией** (см. ниже): `missing=0`, `uncovered=0`, объявленных сирот **0** после чистки; `orphans=3` — состав **уточнён при финальной приёмке §14** (независимая перепроверка Team Lead по git-истории): в `Entries` есть **только `pilot_level`** ([`AstroDriftLocalizationSetup.cs:36`](Assets/Scripts/Core/AstroDriftLocalizationSetup.cs:36)); `title_main`/`title_sub` в `Entries` никогда не были — они только в Shared Data ([`GameTexts Shared Data.asset:19`](Assets/Localizations/GameTexts Shared Data.asset:19) и `:23`). Все три не читаются ни одной нодой/полем → критерий **не достигнут**, дефект P3, удаление/назначение — решение продукта. Фактические правки №1/№2 внесены в [§1.6](Assets/Docs/Localization_TZ.md:95). **Замер Team Lead независимо:** ключей в таблицах 76 → 75 (ru и en), diff по обеим таблицам — **ровно по одной строке** `m_Localized` («ДЕРЕВО» / `TREE`), ссылок на id `1584525502046248` в `Assets/**` — 0
...
| Валидатор: `missing=0`, `uncovered=0`, **`orphans=3` (P3) явной строкой** (состав: `pilot_level` — из `Entries`, `title_main`/`title_sub` — только из Shared Data; нечитаемы все три) | − | + | отложено |
| Ноль grep-нарушений §15 — **буквально 0** (блок A: 15 паттернов = 0; блок B `ProjectSettings`: оба = 0) | − | + | − |
...
| 3 | **3 нечитаемых ключа:** `title_main`, `title_sub` (только Shared Data) + `pilot_level` (в `Entries`) | P3 | Найдено валидатором задачи 8; состав **уточнён при финальной приёмке §14** (независимая перепроверка Team Lead по git-истории). В `Entries` (63 ключа) есть **только `pilot_level`** ([`AstroDriftLocalizationSetup.cs:36`](Assets/Scripts/Core/AstroDriftLocalizationSetup.cs:36)); `title_main`/`title_sub` ([`GameTexts Shared Data.asset:19`](Assets/Localizations/GameTexts Shared Data.asset:19), `:23`) в `Entries` **никогда не были**. Все три имеют строки в обеих таблицах, но не назначены ни одной нодой/полем (§8.1/§8.2 их не мапят); `pilot_level` вытеснен `pilot_level_label`, логотип меню — картинка. Валидатор видит только `title_main`/`title_sub` (непрочитанные ключи таблиц); `pilot_level` не видит — источник №1 исключает `Entries` по построению ([§11.2 п.1](Assets/Docs/Localization_TZ.md:551)). **Решение продукта (милстоун, п.3): удалить пост-милстоуном** — низкий приоритет, бэкап есть; если дизайн захочет сохранить — назначить в UI и добавить в источники валидатора. В милстоуне не делалось (ключи не удаляются без санкции, §0.3). Мерж не блокирует |
...
### Решения продюсера по задаче 7 (приняты, записаны в ТЗ)

- **п.1 Задача 7 ратифицирована** — тег `task7-accepted` в силе; журнал (таблица результата, клоузаут, галочки §10.2, исправление клоузаута задачи 6, Приложение B — 2 строки → ✅) ратифицирован. Снятие заморозки — корректно (по приёмке, не по выдаче).
- **п.2 Санкция на 2 строки в `AddLocalized()` [ратифицирована задним числом](Assets/Docs/Localization_TZ.md:1016).** Обоснование продюсера: критерий (§10.2 п.3) выше шагов (средства); файл — скоуп задачи (там же п.4); правка минимальна и системна (чинит класс дефекта, а не точку). Альтернатива (известный баг в финал) хуже.
- **п.2 Урок «ложное срабатывание нельзя закрывать без прогона» утверждён как правило процесса.** Отдельно зафиксировано: условие задачи 6 (обязательный резолв заголовка дерева) **поймало неверную оценку** — то есть условие сработало; честная правка журнала задним числом квалифицирована как образцовое поведение.
- **п.3 Санкция на выдачу задачи 8 (§11) выдана** — бриф обязан нести именованные пункты (см. ниже, «Состав брифа задачи 8»).
- **п.4 Процессное (жёлтая карточка): финальная приёмка §14 и мерж в `main` НЕ состоятся**, пока в §20 нет записи 4а–4г и двух новых однострочников (i)/(ii). Причина строгости (i): асимметрия «билдеры подписывают `OnUpdateString`, код — нет» доказана в одном месте, поэтому второе кодовое место обязано быть проверено явно, а не «судя по всему».

### Долги журнала 4а–4г + однострочники (i)/(ii) — закрыты разом (жёлтая карточка снята)

4а–4г были записаны в §20 ранее, в блоке [«Уточнения по требованию продюсера (задача 5, п.4а–4д)»](Assets/Docs/Localization_TZ.md:948); по требованию продюсера («закрываем все четыре разом, без новых переносов») ниже они **перезаписаны одним блоком** с независимыми замерами Team Lead, сделанными сегодня, а не по памяти.

**(4а) Строка «единственной подписки `SelectedLocaleChanged`» — снимок продюсера протух.** Формулировка исправлена ещё в `693787e` (блок «Уточнения (задача 5, п.2а–2г)», строка **(п.2а)**): там прямо сказано «не ‹единственная в проекте›» и перечислены **две** живые подписки — [`LanguageService.cs:55`](Assets/Scripts/Core/LanguageService.cs:55) и [`LocalizedText.cs:91`](Assets/Scripts/UI/LocalizedText.cs:91), со снятием второй в задаче 7 (§10.1 п.2). Задача 7 снятие выполнила (файл переписан под `Get`/`GetFormatted`). **Замер Team Lead (независимо, сегодня):** действующих подписок в `Assets/Scripts` = **1** — [`LanguageService.cs:55`](Assets/Scripts/Core/LanguageService.cs:55); [`GameUI.cs:254`](Assets/Scripts/UI/GameUI.cs:254) — условная подписка **для видимого дерева** (§10.1 п.4), симметрично снимается при скрытии и в `OnDestroy`.

**(4б) Определение R11 — дословно (повтор для снимка продюсера; полная запись в §13):** *«При `autoDefineSymbols = 0` прямой вызов `DefineSymbols.ModulesDefineSymbols()` не проверяет `AutoDefinesEnabled()` и вернёт `Localization_yg` на все таргеты; вызовы — `VersionControlWindow.cs:93`, `VersionControlWindow.cs:708`, `ModuleQueue.cs:78`. Митигация — повторный grep после любого ручного импорта модулей YG2.»*

**(4в) Кратность grep-хитов = кратность нод; дублирования нет.** Замер Team Lead (независимо, сегодня, не по памяти): `Assets/Prefabs` — вхождений строки `TypeRoleTag` **16**, из них `m_EditorClassIdentifier: Assembly-CSharp::TypeRoleTag` **16**; [`Game.unity`](Assets/Scenes/Game.unity) — вхождений **3**, из них class-identifier **3**. Итого **19 нод = 16 (префабы) + 3 (сцена)**; все 16 — из §8.1 (что и было целью задачи 5 п.1).

**(4г) Паритет спрайтовых ссылок подтверждён — потерь нет (R5).** Замер Team Lead по `m_Sprite` в **10** префабах `Assets/Prefabs`, `693787e` vs `81566c8` — совпадение **пофайлово**: [`LevelCard`](Assets/Prefabs/Menu/LevelCard.prefab) 4/1↔4/1, [`LevelUpPanel`](Assets/Prefabs/Menu/LevelUpPanel.prefab) 3/1↔3/1, [`UpgradeCard`](Assets/Prefabs/Menu/UpgradeCard.prefab) 2/1↔2/1, [`StartPanel`](Assets/Prefabs/Menu/StartPanel.prefab) 3/3↔3/3, [`MenuButton`](Assets/Prefabs/Menu/MenuButton.prefab) 2/0↔2/0, [`MenuLogo`](Assets/Prefabs/Menu/MenuLogo.prefab) 1/0↔1/0, остальные 0/0↔0/0. **Итог 15 ссылок / 6 пустых до и 15 / 6 после.** `panelBg: {fileID: 0}` при `usePanelBg: 0` — состояние до пересборки (пустой слот существовал и раньше), не потерянная ссылка.

**(i) Хелпер `AstroDriftSceneSetup` для LSE кодовых нод — `OnUpdateString` ПОДПИСЫВАЕТ (асимметрии нет).** Код: [`AddLocalize()`](Assets/Scripts/Core/AstroDriftSceneSetup.cs:569) → [`BindTmpText()`](Assets/Scripts/Core/AstroDriftSceneSetup.cs:578) выполняет `UnityEditor.Events.UnityEventTools.AddPersistentListener(lse.OnUpdateString, handler)` + `SetPersistentListenerState(0, EditorAndRuntime)` ([`:583-584`](Assets/Scripts/Core/AstroDriftSceneSetup.cs:583)), то есть **та же схема, что в [`LevelUpPrefabBuilder.BindTmpText()`](Assets/Scripts/Editor/LevelUpPrefabBuilder.cs:248) и [`MenuPrefabBuilder`](Assets/Scripts/Editor/MenuPrefabBuilder.cs:390)**. **Независимое подтверждение по факту сборки, а не по коду:** в [`Game.unity`](Assets/Scenes/Game.unity) ровно **11** компонентов `LocalizeStringEvent` и ровно **11** вызовов `m_MethodName: set_text` со `m_CallState: 1` (EditorAndRuntime) — 11/11; в `Assets/Prefabs` — **16/16**. Ноды паузы/смерти/continue, созданные этим хелпером, строку получают.

**(ii) `AddLocalized` вызывается один раз на ноду — накопление лямбда-слушателей исключено.** Вызов ровно один: [`GameUI.cs:222`](Assets/Scripts/UI/GameUI.cs:222); нода `Title` создаётся там же и один раз, метод защищён гардом `if (_treePanel != null) return;` ([`BuildTreePanel()`](Assets/Scripts/UI/GameUI.cs:193)) на повторный `Init`; `AddComponent<LocalizeStringEvent>()` — один раз на ноду, лямбда добавляется один раз ([`GameUI.cs:1205`](Assets/Scripts/UI/GameUI.cs:1205)). Накопления нет по построению.

### Состав брифа задачи 8 (§11) — обязательные именованные пункты

1. **§11.1** — удаление сироты «ДЕРЕВО» (id `1584525502046248`) из `GameTexts_ru`/`GameTexts_en`, **после бэкапа таблиц**.
2. **§11.2** — валидатор: 6 источников ключей + coverage чарсета; трактовка покрытия — **объединение назначенных шрифтов** ([§11.2 п.6](Assets/Docs/Localization_TZ.md:556), решение записано).
3. **Cleanup–8:** 9 диагностических логов задачи 3 **с перечислением `file:line`** (сверка с «логами задачи 4» — [фантом](Assets/Docs/Localization_TZ.md:979), чтобы ничего не потерять) + осиротевшая YAML-строка сцены `shieldTextLse: {fileID: 0}` ([`Game.unity:4868`](Assets/Scenes/Game.unity:4868)) + **подтверждение, что поле `shieldTextLse` уже удалено в задаче 7**.
4. **Закрытие долгов журнала** — идёт пунктом брифа, не сноской отчёта.
5. **R10/R5 при пересохранении сцены:** diff минимален — только осиротевшая строка.
...
- **Находка журнала (правка §1.6 — две итерации):** §1.6 неточно утверждала, что `title_main`/`title_sub`/`pilot_level` «созданы вне `Entries`». Правка №1 (задача 8) ошибочно заявила, что все три **в** `Entries`. Правка №2 (финальная приёмка §14, независимая перепроверка Team Lead): **в `Entries` (63 ключа) есть только `pilot_level`** ([`AstroDriftLocalizationSetup.cs:36`](Assets/Scripts/Core/AstroDriftLocalizationSetup.cs:36)); **`title_main`/`title_sub` в `Entries` никогда не были** — git-история (`d9bf5e1`, `d90f06e`, `f0155fd`, `0a920d0`, HEAD) даёт ноль вхождений; они существуют только в [`GameTexts Shared Data.asset:19`](Assets/Localizations/GameTexts Shared Data.asset:19) и `:23`. Все три имеют строки в обеих таблицах, но не читаются ни одной нодой/полем; `pilot_level` вытеснен `pilot_level_label`, логотип меню — картинка. **Удаление/назначение — решение продукта**, в милстоуне не делалось (вне санкции, §0.3). Долг продукта — ниже.
...
### Решения продюсера по задаче 8 (приняты, записаны в ТЗ)

- **п.1 Задача 8 ратифицирована** — тег `task8-accepted` в силе (цепочка `0a920d0` → `efc914f` → `75ef64d`). Журнал ратифицирован: результат, клоузаут, §11.3, §1.6, пост-милстоун P3, Приложение B. Именованные пункты брифа выполнены все, включая «закрытие долгов — пунктом брифа» и R10/R5.
- **п.2 Санкция 1 — квалификация «0 сирот»: РАТИФИЦИРОВАНО поведение (стоп + эскалация), НЕ ратифицирована формулировка «зелёный».** Запись `§11.3 п.1` в редакции `75ef64d` несёт «✅ с квалификацией» — этого достаточно; дополнительно [§14](Assets/Docs/Localization_TZ.md:601) **обязана показать `orphans=3` (P3) явно в финальной матрице**, а не прятать под «0». Заведено как **пост-милстоун дефект P3** (низкий приоритет). В милстоуне не удаляем.
- **п.3 Санкция 2 — правка §1.6 задним числом РАТИФИЦИРОВАНА как фактическая.** ТЗ утверждало ложный факт («созданы вне `Entries`»); факт — внутри `Entries`, но не читаются. Правка — **коррекция диагноза, не расширение скоупа**. Откат не требуется.
- **п.4 Решение по P3 — за продуктом, вне милстоуна.** Рекомендация продюсера: удалить после проверки дизайна (ключи не используются, бэкап есть). Альтернатива: назначить в UI и добавить в источники валидатора. До решения — **P3 открыт**.
- **п.5 Жёлтая карточка СНЯТА.** 4а/4б/4в/4г + (i)/(ii) записаны в §20 коммитом `0a920d0` **до** выдачи задачи 8 — блокировка §14 и мержа снята. Процессная фиксация: переносы однострочников больше не повторять — закрытие долгов теперь **пункт брифа**, что и сработало.
- **п.6 Санкция на финал выдана:** приёмка милстоуна [§14](Assets/Docs/Localization_TZ.md:601) + мерж в `main`.

### Ограничения инструментов — записаны как уроки (решение продюсера, п.6)

- **Валидатор пачкает `LiberationSans SDF - Fallback.asset`** при аддитивном открытии сцены (TMP-автозаполнение глифов добрало греческий: +56/−6). В коммит не попало, откачено `git checkout --`. **Правило:** после прогона валидатора — проверять `git status` и откатывать.
- **`validate_script` не видит часть CS-ошибок.** Первая итерация валидатора дала `CS0246` (`StringTableCollection` в неверном namespace), при этом `validate_script` вернул 0 диагностик. **Правило:** при сомнении смотреть `~/Library/Logs/Unity/Editor.log`, а не доверять только инструменту.

### Состав брифа финальной приёмки §14 (обязательные пункты)

1. **Полная матрица [§14](Assets/Docs/Localization_TZ.md:601)** — активный профиль **RuStore**, **Editor**; **ЯИ отложено (F5)**. Метрики SDF: размер, PopMode, MultiAtlas, атлас, PointSize, глифы, Padding. Ключи **76 → 75**. Валидатор: `missing=0`, `uncovered=0`, **`orphans=3` явной строкой (P3)**.
2. **Полный обход экранов RU/EN явным списком** (долг 4д, решение задачи 6 п.2): **старт / смерть / пауза / оверлей перка / дерево разблокировок × RU + EN**, с **дословными строками заголовков** — включая заголовок дерева (`РАЗБЛОКИРОВКИ ПИЛОТА` / `PILOT UNLOCKS`), закрывающий наблюдение задач 6/7.
3. **Пост-милстоун список:** P1 `menuUpgradeBtn: {fileID: 0}`, P2 `Sheet.png`, P3 3 ключа.
4. **R10/R11 и логи — подтвердить grep'ом:** R10 — снапшоты профилей не изменены; R11 — `Localization_yg` = 0; логи задачи 3 удалены (`[Lang]`/`[Boot]` = 0).

**Оговорка для п.1:** измеренные метрики SDF берутся из **ассета** (ин-движок), не из сборки, поэтому колонка «ЯИ WebGL» для них = «отложено» по смыслу F5 — билды не собираются; это не «−» (не запускалось), а именно отложено до первой сборки ЯИ после милстоуна.

### Результат финальной приёмки милстоуна (§14) — коммитов не потребовал (HEAD `97bc11d`)

Финальная приёмка выполнена исполнителем в режиме Play Mode ин-движка на активном профиле **RuStore**; проверено Team Lead по git-статусу и grep'ом. Постоянных правок кода/ассетов **нет** (временный дебаг-хук локали снят, `git status` пуст до и после).

| Строка [§14](Assets/Docs/Localization_TZ.md:601) | RuStore (активный профиль) | Editor | ЯИ WebGL (отложено, F5) |
|---|---|---|---|
| Финальный обход экранов RU/EN — явным списком | ✅ | ✅ | отложено |
| Холодный старт ru/en → верная локаль | ✅ (systemLang) | ✅ | отложено |
| Неподдерживаемый язык → ru (D1) | ✅ | − | отложено |
| Все экраны RU/EN без визуальных отличий (вкл. B1-строки) | ✅ | ✅ | отложено |
| Смена локали mid-session (временный дебаг-хук) | − | ✅ | − |
| **Метрики SDF до/после** | ✅ | ✅ | − |
| Валидатор `missing=0`, `uncovered=0`, `orphans=3` (P3) | − | ✅ | отложено |
| Ноль grep-нарушений §15 | − | ✅ **буквально 0** (после коммита `e61863d`) | − |

**Не строки §14, но обязательные пункты брифа (проверены отдельно):** компиляция — 0 CS-ошибок на активном профиле RuStore, ItchIO и YandexGames (ин-движок, `CS\d{4}` = 0); ключи таблиц **76 → 75** в обеих локалях (diff — ровно одна строка `m_Localized` «ДЕРЕВО»/`TREE`).

**Метрики SDF (ассет [`Montserrat-SemiBold SDF.asset`](Assets/Fonts/Montserrat-SemiBold SDF.asset), ин-движок, до/после):**

| Метрика | До (baseline `baseline-loc-migration`) | После (HEAD) | Статус |
|---|---|---|---|
| Размер файла | 36 339 195 Б (34.66 МБ) | 4 431 205 Б (4.23 МБ) | ✅ шаг 2 лестницы (F6); **+69 Б** к `88448c4` (4 431 136) — добавлена ссылка на fallback |
| `m_AtlasPopulationMode` | Dynamic (1) | Static (0) | ✅ |
| `m_IsMultiAtlasTexturesEnabled` | on (1) | off (0) | ✅ |
| Атлас | 2048×2048 | 2048×1024 (1 текстура) | ✅ (шаг 2) |
| `m_PointSize` | 90 | 60 | ✅ |
| Глифов (`m_CharacterTable`) | 99 | 443 | ✅ |
| `m_AtlasPadding` | 9 | 9 | ✅ (не менялся) |
| `m_AtlasRenderMode` | 4165 (SDFAA) | 4165 (SDFAA) | ✅ |
| `m_FallbackFontAssetTable` | `[]` | 1 ссылка (guid `2e498d1c8094910479dc3e1b768306a4`) | ✅ (задача 4) |

**Правка отчёта исполнителя (принята Team Lead):** в отчёте финальной приёмки колонка «До» была ошибочно заполнена состоянием **после задачи 1** (4 431 136 Б / Static / 2048×1024 / PS60 / 443), а не baseline. **Факт по git** (`baseline-loc-migration` → HEAD): 36 339 195 → 4 431 205 Б, Dynamic→Static, MultiAtlas on→off, 2048×2048→2048×1024, PS 90→60, глифов 99→443. Восемь метрик §14 подтверждены ин-движком и git-историей; качественный вывод не меняется.

**Обход экранов RU/EN (долг 4д, решение задачи 6 п.2 — закрыт):** пройдены **старт / смерть / пауза / оверлей перка / дерево разблокировок × RU + EN** с дословными заголовками — старт `РЕКОРД`/`BEST`, `ТАП — СТАРТ`/`TAP TO START`; смерть `СЧЁТ`/`SCORE`, `НОВЫЙ РЕКОРД!`/`NEW BEST!`, `ПРОДОЛЖИТЬ`/`CONTINUE`, `ЗА ПРОСМОТР РЕКЛАМЫ`/`WATCH AD`, `В МЕНЮ`/`MENU`, `УРОВЕНЬ 0 → 2`/`LEVEL 0 → 2`, `РАЗБЛОКИРОВАНО:`/`UNLOCKED:`; пауза `ПАУЗА`/`PAUSE`; оверлей перка `УРОВЕНЬ ПОВЫШЕН!`/`LEVEL UP!`, `ВЫБЕРИ УЛУЧШЕНИЕ`/`CHOOSE AN UPGRADE`, `НОВОЕ`/`NEW`, `ОБНОВИТЬ ВЫБОР`/`REROLL`; дерево `РАЗБЛОКИРОВКИ ПИЛОТА`/`PILOT UNLOCKS` (+21 уровень). **Заголовок дерева резолвится** — наблюдение задач 6/7 закрыто.

**Grep-подтверждения (перепроверено Team Lead в корректных скоупах; отчёт исполнителя был неточен — см. ниже):**

- §15, блок A (`Assets/Scripts` + `Assets/_Platform` + `Assets/Scenes` + `Assets/Prefabs`): **14 паттернов = 0**, включая `L10n.Bind`, `LocalizedTextUI`, `AstroDriftLanguageBridge`, `YG2.lang`, `YG2.onSwitchLang`, `onCorrectLang`, `LanguageYG`, `AutoTranslateLangs`, `fontsTMP`, `Application.onBeforeRender`, `LanguageChanged`, `NotifyLanguageChanged`, `titleKey`, `descKey`.
- §15, блок B (`ProjectSettings/ProjectSettings.asset`): `AutoTranslateLangs_yg` = **0**, `Localization_yg` = **0**.
- **Остаток, требующий решения (не закрыт):** `CorrectLang` = **1** — док-комментарий [`LanguageService.cs:19`](Assets/Scripts/Core/LanguageService.cs:19) («отклонение от поведения удалённого моста/CorrectLang»), **вызовов 0**. По буквальному контракту §15 («ноль вхождений») критерий **не выполнен**; по прецеденту `L10n.Bind` (задача 6/7 — критерий применён к **вызовам**, док-строка снята задачей 7 для буквального нуля) — либо снимается одной строкой, либо квалифицируется решением продюсера.
- **Ошибка отчёта исполнителя (исправлена):** исполнитель сообщил «§15 — всё = 0» и «`Localization_yg` = 0»; фактически он проверял `Localization_yg` **не в том скоупе** (`Assets/PluginYourGames`, где 30 вхождений — код самого плагина YG2, тогда как §15 требует ноль только в `ProjectSettings.asset`) и **пропустил** остаточный `CorrectLang`.
- Логи задачи 3 (`[Lang]`/`[Boot]`) = **0** (удалены в задаче 8); снапшоты `Build Profiles` — дифф пуст (R10 замкнут в части наблюдения).

**Находка финальной приёмки (§1.6 — правка №2):** правка §1.6 №1 (задача 8) ошибочно заявила, что `title_main`/`title_sub`/`pilot_level` **все три** в `Entries`. Факт (git-история `d9bf5e1`, `d90f06e`, `f0155fd`, `0a920d0`, HEAD): в `Entries` есть **только `pilot_level`**; `title_main`/`title_sub` в `Entries` никогда не были — они только в Shared Data. Правки внесены в [§1.6](Assets/Docs/Localization_TZ.md:95), [§11.3](Assets/Docs/Localization_TZ.md:562), строку P3 (§20) и [§14](Assets/Docs/Localization_TZ.md:609). **Состав `orphans=3` уточнён:** 1 ключ из `Entries` + 2 ключа только из Shared Data.

### Клоузаут финальной приёмки §14

- Милстоун **принят Team Lead с двумя квалификациями**; постоянных правок не потребовал — финальная приёмка не создаёт коммит (R5 не нарушен, R10 подтверждён — дифф профилей пуст, R11 — `ProjectSettings` чист, логи задачи 3 удалены).
- Долг **4д** (обход экранов RU/EN явным списком) закрыт полностью — заголовок дерева `РАЗБЛОКИРОВКИ ПИЛОТА`/`PILOT UNLOCKS` резолвится (наблюдение задач 6/7 закрыто).
- Расхождение ТЗ↔факт по §1.6 **устранено правкой №2** — решение о самих ключах остаётся за продуктом (P3).
- **Метрики SDF:** таблица переписана по git-факту (`baseline-loc-migration` → HEAD: 36 339 195 → 4 431 205 Б, Dynamic→Static, MultiAtlas on→off, 2048×2048→2048×1024, PS 90→60, глифов 99→443). Отчёт исполнителя смешивал baseline с состоянием после задачи 1; качественный вывод §4.3 не меняется.
- **§15 — квалификация СНЯТА, критерий выполнен буквально:** решением продюсера остаток `CorrectLang` (1 док-строка, [`LanguageService.cs:19`](Assets/Scripts/Core/LanguageService.cs:19), вызовов 0) снят одной правкой — коммит **`e61863d`** (родитель `3f315b7`), 1 файл, +1/−1, изменён только комментарий (исполняемый код — нулевой diff). После правки: блок A §15 = **15/15 паттернов по 0**, блок B `ProjectSettings` = 0/0, `git status` пусто, 0 CS-ошибок на RuStore/ItchIO/YandexGames. Правка поведение не меняет — приёмка §14 остаётся в силе.
- **Все три условия закрыты** (решения продюсера по милстоуну): правка §1.6 №2 ратифицирована; `CorrectLang` снят; P3 подтверждён как пост-милстоун (мерж не блокирует).
- Готовность: **READY к мержу в `main`** — блокировок нет.

### Решения продюсера по милстоуну (ИТОГ — ПРИНЯТ)

1. **Правка §1.6 №2 — РАТИФИЦИРОВАНА** как замена неверной правки №1 (факт по git-истории: `d9bf5e1`/`d90f06e`/`f0155fd`/`0a920d0`/HEAD — 0 вхождений `title_main`/`title_sub`). В `Entries` (63 ключа) — только `pilot_level`; `title_main`/`title_sub` — только Shared Data `:19`/`:23`. Синхронизация в §1.6, §11.3, P3, §14, Приложении B ратифицирована. **Правка №1 считается отменённой и заменённой №2.**
2. **`CorrectLang` — СНЯТЬ ОДНОЙ СТРОКОЙ** (как `L10n.Bind` в задаче 7), выбран чистый 0 вместо квалификации. Исполнено коммитом `e61863d`; после правки §15 блок A = 0 буквально, `git status` чисто, 0 CS-ошибок на 3 профилях.
...
| `Assets/Docs/Localization_TZ.md:95` | **Сделано, уточнено дважды**: правка §1.6 №1 (задача 8) — ключи исключены из перечня «созданы вне `Entries`»; правка №2 (финальная приёмка §14) — в `Entries` есть только `pilot_level`, `title_main`/`title_sub` только в Shared Data | 8 ✅ |
