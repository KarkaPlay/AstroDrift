...
| Ноль grep-нарушений §15 (блок A: 14 паттернов = 0; блок B `ProjectSettings`: оба = 0; **остаток `CorrectLang` = 1 док-строка, вызовов 0** — требует решения) | − | + с квалификацией | − |
...
| Ноль grep-нарушений §15 | − | ✅ **с квалификацией** (`CorrectLang` = 1 док-строка, вызовов 0) | − |
...
- **§15 — квалификация, не закрыт буквально:** все паттерны блоков A/B = 0, кроме **`CorrectLang` = 1 док-строка** ([`LanguageService.cs:19`](Assets/Scripts/Core/LanguageService.cs:19), вызовов 0). По прецеденту `L10n.Bind` — либо снять строку, либо квалифицировать решением продюсера.
- **Три условия к решениям продюсера** (мержу не блокирующие, но требующие явной позиции):
  1. **Ратификация правки §1.6 №2** (фактическая коррекция диагноза — по прецеденту решения по задаче 8, п.3).
  2. **Решение по `CorrectLang`** — снять док-строку одной правкой либо принять буквальную квалификацию §15 (как с `orphans=3`).
  3. **P3** — удалить/назначить 3 ключа (пост-милстоун, решение продукта).
- Готовность: **READY для мержа в `main`** при принятии квалификаций выше. Блокировка мержа, снятая задачей 8, сохраняется снятой.
