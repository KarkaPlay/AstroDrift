...
| **Версия** | 2.3 (финальная вычитка: F1–F4 + 2 уточнения) |
| **Дата** | 2026-09-18 |
| **Заменяет** | `Docs/Localization_TZ.md` v2.2 |
...
5. Прогнать все экраны RU/EN + тестовую строку: `— → − · «» ı ş ğ İ` (покрывает B1 и турецкие символы).
...
- [ ] Population Mode = Static, MultiAtlas = off, Padding = 9, Point Size = 60, SDFAA, Kerning off (проверить в инспекторе/YAML);
- [ ] Все экраны RU/EN визуально без изменений, **вкл. строки из B1** (скриншоты до/после: стартовый экран, Death с `level_up_line`, оверлей перка с EN-описаниями). Даунсемпл point size 90→60: ожидаемый дифф — нулевой (опора: Bold@59 уже рендерит крупные кегли); явные артефакты = провал приёмки;
- [ ] Тестовая строка `— → − · «» ı ş ğ İ` рендерится без квадратов;
- [ ] Замер веса билда до/после — в матрицу §14 (WebGL и RuStore).
...
- [ ] Проект компилируется **во всех билд-профилях**, вкл. YandexGames (B3); окно настроек YG2 открывается;
- [ ] Источник кода языка на ЯИ зафиксирован (вариант 1 или 2 гейта G1) и **проверен в тестовой сборке ЯИ**.
...
- [ ] Все экраны RU/EN визуально как было (скриншоты до/после, вкл. паузу и оверлей перка);
...
| R5 | Потеря ссылок при пересборке префабов | Билдеры + `Setup Scene UI` в одном коммите; правило владения §8.0; скриншоты до/после |
...
| R9 | `Docs/` целиком в `.gitignore` (`:12`) — ТЗ не под версионным контролем: риск потери/расхождения | Вынести `Docs/Localization_TZ.md` из ignore или продублировать в трекаемую папку (ответственный — продюсер, до старта задачи 1) |

---

## 14. Приёмочная матрица

| Проверка | ЯИ WebGL | RuStore | Editor |
|---|---|---|---|
| Холодный старт ru/en → верная локаль | + | + (systemLang) | + |
| Неподдерживаемый язык → ru (D1) | + | + | − |
| Все экраны RU/EN без визуальных отличий (вкл. B1-строки) | + | + | + |
| Смена локали mid-session (временный дебаг-хук) | − | − | + |
| **Замер веса билда до/после** (SDF; WebGL и RuStore) | + | + | − |
| Валидатор зелёный (ключи + чарсет) | − | − | + |
| Ноль grep-нарушений §15 | − | − | + |

> Вес билда — замером, не арифметикой дельты файла: 26.65 МБ YAML-таблиц хорошо жмутся,
> реальная экономия станет известна только по факту. SDF влияет и на RuStore — замер на обоих.
...
- [ ] **Гейт G1:** источник кода языка (перенос `LangRequest_js` на `i18n.lang` / язык браузера) — что проверено в тестовой сборке ЯИ, то записано;
